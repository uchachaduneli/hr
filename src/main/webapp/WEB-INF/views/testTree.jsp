...
<script>
  ...
  app.controller("TreeController", function ($scope, $http, $filter, $location) {
    ...
    $scope.tmpCandidates = [];
    $scope.tmpVoters = [];
    // oldVoters-shi iyreba redaqtirebis dros mimdinare shemfaseblebi
    $scope.oldVoters = [];
    $scope.candidates = [];
    $scope.voters = [];
    $scope.votePair = [];
    ...
    $scope.addCandidates = function () {
      if ($scope.edit) {
        angular.forEach($scope.oldVoters, function (v, k) {
          ...
          var _selected = $filter('filter')($scope.votePair, {voterId: v}, true);
          angular.forEach(_selected, function (val, key) {
        ...
        if (selectedVoter.length == 0) {
          ...
          if (selectedPersonal.length != 0) {
            ...
            angular.forEach($scope.tmpCandidates, function (value, key) {
              var selectedCandidate = $filter('filter')($scope.personals, {id: value}, true);
              if (selectedCandidate.length != 0) {
                var tmp = {};
                tmp.testId = $scope.currentTestId;
                tmp.voterId = v;
                tmp.candidateId = value;
    ...
    $scope.editCandidate = function (voterId) {
      if (voterId == 0) {
        ...
        angular.forEach(voter, function (v, k) {
          ...
          $('#jstree2').jstree(true).select_node(v.candidateId);
        });
    ...
    $scope.changeVoter = function (voterId) {
      ...
      angular.forEach(selected, function (v, k) {
        var s = {};
        s.id = v.candidateId;
        var _sel = $filter('filter')($scope.personals, {id: v.candidateId}, true);
        s.text = _sel[0].firstName + ' ' + _sel[0].lastName;
        ...
        <div class="panel-body maxPanel">
          ...
          <div class="col-md-6">
            <div class="form-group col-md-10">
              ...
              <select class="form-control input-sm" ng-model="candidate.testId">
